...
class IndexController extends HomeBaseController{
	...
	public function index(){
        ...
        //近期作品
        $timesVideo=M('Video')->order('create_time desc')->limit(5)->field('id,title,thumb,brand,times,create_time,playbacknum,path')->select();
        //品牌视频
        $brandVideo=M('Video')->order('create_time desc')->field('id,title,thumb,brand,times,create_time,playbacknum,path')->group('brand')->limit(6)->select();
        // p($brandVideo);exit;
        //产品视频
        $industryVideo=M('Video')->order('create_time desc')->field('id,title,thumb,brand,times,create_time,playbacknum,path')->group('industry')->limit(8)->select();
        //行业新闻
        $news=M('Post')->order('browse desc')->limit(3)->field('id,title,thumb,abstract')->select();
        //最近动态
        $post=M('Post')->order('create_time desc')->limit(5)->field('id,title,thumb,abstract')->select();
        $_title='首页';
        $res=[
            'banner'=>$banner,
            'timesVideo'=>$timesVideo,
    ...
    public function logout(){
        session('user',null);
        $this->success('退出成功、前往登录页面',U('/admins'));
    }
